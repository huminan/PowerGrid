...
class Richardson(DistributedLinearPowerGrid):
  ...
  #       NULL
  #############################################################
  def estimator(self, main_period=MAIN_LOOP_PERIOD, gamma_period=GAMMA_MAX_EIGEN_PERIOD, is_plot=True):
    ...
    for i in range(self.nodes_num):
      thread_nodes.append(threading.Thread(target=self.__estimator, args=(i, lock_con, main_period, gamma_period)))
  ...
  #       NULL
  #############################################################
  def __estimator(self, num, lock_con, main_period, gamma_period):
    import threading
